...
public class Jeu{
	...
	public void lancer() {
		...
		String s = server.getMessage("01") + ihm.afficherPlateau(plateau);
		for(Client c : server.getLstCli())
			server.sendMessage(c, s);

		s = server.getMessage("10") + " (" + this.alJoueurs.get(0).getCouleur().toUpperCase() + ") :\nenvoie au serveur chaine\n ";
		server.sendMessage(server.getLstCli().get(0), s);

		this.alJoueurs.get(0).setTour(true);
	...
	public void executeTurn(int client, String msg){
		...
		boolean plein = false ;
		boolean enCours = true;
		Joueur thePlayer = this.alJoueurs.get(client);
		Joueur autreJoueur = this.alJoueurs.get((client + 1) % 2);
		if(thePlayer.getNbTwist() > 0) {
			thePlayer.setTour(false);
            this.alJoueurs.get((client + 1) % 2).setTour(true);
			thePlayer.setTour(true);
            ...
            String s = server.getMessage("10") + " (" + this.alJoueurs.get(0).getCouleur().toUpperCase() + ") :\nenvoie au serveur chaine\n ";
            server.sendMessage(server.getLstCli().get((client + 1) % 2), s);
        }
		thePlayer.setTour(false);
	...
	public boolean setPosition(int numJoueur, String pos) {
		if(this.alJoueurs.get(numJoueur).getTour()){
			this.pos = pos;
			return true;
		}
		else
